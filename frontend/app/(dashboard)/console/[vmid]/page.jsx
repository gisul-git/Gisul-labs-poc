  ...
  return (
    <div className="flex flex-col h-screen">
      <div className="flex items-center gap-4 px-4 py-2 bg-gray-900 border-b border-gray-800">
        ...
        <span className={`text-xs px-2 py-0.5 rounded-full ${status === 'Connected' ? 'bg-green-900 text-green-300' : status === 'Error' ? 'bg-red-900 text-red-300' : 'bg-gray-800 text-gray-400'}`}>
          {status}
        </span>
        <div className="ml-auto flex gap-2">
          <button className="btn-ghost text-xs" onClick={() => rfbRef.current?.sendCtrlAltDel()} disabled={status !== 'Connected'}>Ctrl+Alt+Del</button>
          <button className="btn-ghost text-xs" onClick={() => canvasRef.current?.requestFullscreen?.()}>Fullscreen</button>
        </div>
      </div>
      {error && <div className="bg-red-900/50 border-b border-red-800 text-red-300 text-sm px-4 py-2">{error}</div>}
